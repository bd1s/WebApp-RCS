...
const ReunionForm = ({ reunion, onClose, isCreating }) => {
  const [departments, setDepartments] = useState([]); // Stocke les départements
  const [selectedDepartment, setSelectedDepartment] = useState(''); // Département sélectionné
  const [doctorants, setDoctorants] = useState([]); // Stocke les doctorants du département sélectionné
  const [availableDoctorants, setAvailableDoctorants] = useState([]); // Liste des doctorants disponibles pour la réunion
  ...
  useEffect(() => {
    async function fetchDepartments() {
      try {
        const response = await axios.get('/reunions/departements');
        setDepartments(response.data);
      } catch (error) {
        console.error('Failed to fetch departments:', error);
        // Affichez une notification ou un message d'erreur à l'utilisateur ici
      }
    }
  ...
  useEffect(() => {
    if (selectedDepartment) {
      // Exemple de récupération des doctorants pour le département sélectionné
      async function fetchDoctorants() {
        const response = await axios.get(`/reunions/doctorants?department=${selectedDepartment}`); // Modifier avec votre API réelle
  ...
  const handleDoctorantSelect = (selected) => {
    setAvailableDoctorants(
      selected.map(id => doctorants.find(doctorant => doctorant.id === id))
    );
  };
      ...
      <Formik
        ...
        onSubmit={async (values) => {
          const formattedValues = {
            ...
            heure_debut: moment(values.heure_debut, 'HH:mm').format('HH:mm'),
            heure_fin: moment(values.heure_fin, 'HH:mm').format('HH:mm'),
            doctorants: availableDoctorants.map(d => d.id) // Associe les doctorants sélectionnés
          };
          if (isCreating) {
            await createReunion(formattedValues);
          } else {
        ...
        {dept}
            ...
            <FormControl mt={4}>
              ...
              <Select
                ...
                onChange={(e) => handleDoctorantSelect([...e.target.selectedOptions].map(option => option.value))}
                value={availableDoctorants.map(d => d.id)}
                multiple
              >
